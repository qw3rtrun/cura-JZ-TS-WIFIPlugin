...
from PyQt5.QtCore import QUrl, QObject, QByteArray, pyqtSlot, QTimer, QSize, Qt
from PyQt5.QtGui import QIcon, QStandardItem, QStandardItemModel
from PyQt5.QtWidgets import QFileDialog, QInputDialog
from PyQt5.QtWidgets import QListView, QStatusBar, QLineEdit, QVBoxLayout, QWidget, QHBoxLayout, QPushButton, QLabel
...
class PluginMain(QObject):
    def __init__(self):
        ...
        self.ui_items_name = {"lb_ip", "txt_printer_ip", "btn_connect", "btn_disconnect", "lb_printer_name",
                              "lb_printer_state"
            , "lb_printer_t0", "lb_printer_t1", "lb_printer_bed", "btn_preheat", "btn_cooldown", "btn_sd", "lb_gcode",
                              "txt_gcode", "txt_log"
            , "btn_send", "pgb_printing_progress", "lb_printing_progress", "btn_pause", "btn_resume", "btn_stop",
                              "btn_off", "lb_printing_filename", "lb_printing"
            , "btn_upload", "lv_ip_list", "img_edit_printer_name", "btn_gcode_help", "btn_homexyz", "btn_t0", "btn_t1"}

        self.ui_items_on_normal_only = {"btn_preheat", "btn_cooldown", "btn_sd", "lb_gcode", "txt_gcode", "txt_log", "btn_send",
                                        "btn_upload", "btn_gcode_help"
            , "btn_homexyz", "btn_t0", "btn_t1"}

        self.ui_items_on_printing_only = {"pgb_printing_progress", "lb_printing_progress", "btn_pause", "btn_resume",
                                          "btn_stop", "btn_off"
            , "lb_printing_filename", "lb_printing"
                                          }

        self.dialog_uploading_ui_items_name = {"pgb_progress", "lb_remain_time", "btn_cancel", "lb_uploading_filename",
                                               "ck_auto_start_print"
            , "lb_speed", "btn_upload"
                                               }
        ...
        self.btns_name = {"btn_preheat", "btn_cooldown", "btn_sd", "btn_send", "btn_upload", "txt_gcode", "txt_log", "btn_homexyz",
                          "btn_t0", "btn_t1"}
    ...
    def edit_name(self):
        if self.print_id != 0:
            name, okPressed = QInputDialog.getText(None, "Edit", "Printer Name", QLineEdit.Normal,
                                                   self.lb_printer_name.property("text"))
            if okPressed:
                if name == "":
    ...
    def update_history_ip(self):
        ...
        for i in range(0, model.rowCount()):
            if i < data_len:
                model.setItemData(model.createIndex(i, 0), {1: self.ip_list[i], 0: "True"})
            else:
    ...
    def save_user_data(self):
        ...
        with open(file_path, 'w') as f:
            json.dump({"last_ip": self.last_ip, "last_upload_dir": self.last_upload_dir, "ip_list": self.ip_list,
                       "name_dict": self.name_dict}, f)
    ...
    def update_status(self):
        ...
        self.lb_printer_t0.setProperty('text', "Extruder1 %d/%d" % (
        self.temp_nozzle0, self.temp_nozzle_target0 if self.is_nozzle_heating0 else 0))

        if self.nozzle_count > 1:
            self.lb_printer_t1.setProperty('visible', True)
            self.lb_printer_t1.setProperty('text', "Extruder2 %d/%d" % (
            self.temp_nozzle1, self.temp_nozzle_target1 if self.is_nozzle_heating1 else 0))
        else:
            self.lb_printer_t1.setProperty('visible', False)

        self.lb_printer_bed.setProperty('text', "Bed %d/%d" % (
        self.temp_bed, self.temp_bed_target if self.is_bed_heating else 0))
    ...
    def show_upload(self):
        openfile_path, _ = QFileDialog.getOpenFileName(None, 'Choose File',
                                                       self.last_upload_dir if self.last_upload_dir else ''
                                                       ,
    ...
    def on_upload_progress(self, bytesSent, bytesTotal):
        ...
        if bytesTotal > 0:
            ...
            self.dialog_uploading_ui["lb_remain_time"].setProperty('text', 'Remain Time: %d Second' % (
                int((bytesTotal - bytesSent) / speed)))
            ...
            if reply_body.find("{") >= 0:
                ...
                if obj["c"] == 0:
                    ...
                    if start_printing:
                        ...
                        Message(os.path.basename(self.upload_file_path) + " is uploaded to your printer!",
                                lifetime=8).show()
                else:
                    code = obj["c"]
    ...
    def start_upload(self, save_path, file_path):
        ...
        self.upload_http_request = HttpRequest(self, "http://%s/upload" % (self.printer_ip), {}, self.on_upload_result,
                                               self.on_upload_error, data, self.on_upload_progress)
        self.upload_http_request.set_header("path", save_path)
        self.upload_http_request.run()
        self.dialog_uploading_ui["lb_uploading_filename"].setProperty('text',
    ...
    def connect_printer(self):
        ...
        if len(ip) > 0 and self.is_ip(ip) and not self.cur_http_request:
            http_request = HttpRequest(self, "http://%s/connect" % (ip), {"ip": ip}, self.on_connect_result,
        ...
        if reply_body != None and len(reply_body) > 0:
            ...
            if reply_body.find("DATA") == 0:
                ...
                if len(status) == 18:
                    ...
                    self.txt_log.setProperty("text", reply_body)
                    self.update_status()
            ...
            try:
                if len(self.http_send_queue) > 0:
                    ...
                    if t == "cmd":
                        cmd = req.get("cmd")
                        data = req.get("data")
                        http_request = HttpRequest(self, "http://%s/cmd" % (self.printer_ip), {}, self.on_cmd_result,
                                                   self.on_http_error)
                        ...
                        data = req.get("path")
                        http_request = HttpRequest(self, "http://%s/filelist" % (self.printer_ip), {},
                                                   self.on_filelist_result, self.on_http_error)
                    ...
                    if time.time() - self.last_get_status_time >= 1:
                        ...
                        http_request = HttpRequest(self, "http://%s/status" % (self.printer_ip), {},
                                                   self.on_query_status_result, self.on_http_error)
