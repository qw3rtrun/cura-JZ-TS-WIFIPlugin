...
class JzTsWifiOutputDevicePlugin(OutputDevicePlugin): #We need to be an OutputDevicePlugin for the plug-in system.
    ##  Called upon launch.
    #
    #   You can use this to make a connection to the device or service, and
    #   register the output device to be displayed to the user.
    def start(self):
        self.getOutputDeviceManager().addOutputDevice(JzTsWifiOutputDevice()) #Since this class is also an output device, we can just register ourselves.
    ...
    def stop(self):
        self.getOutputDeviceManager().removeOutputDevice("JzTsWifi_output_device") #Remove all devices that were added. In this case it's only one.

class JzTsWifiOutputDevice(OutputDevice): #We need an actual device to do the writing.
    def __init__(self):
        super().__init__("JzTsWifi_output_device") #Give an ID which is used to refer to the output device.

        #Optionally set some metadata.
        self.setName("JzTsWifi Output Device") #Human-readable name (you may want to internationalise this). Gets put in messages and such.
        self.setShortDescription("Send To Printer") #This is put on the save button.
        self.setDescription("Send gcode file to your printer via WIFI")
    ...
    def do_snap(self,gfile):
        ...
        JzTsWifiStage.JzTsWifiStageIns.setUploadFileName(gjz)
        CuraApplication.getInstance().getController().setActiveStage(JzTsWifiStage.JzTsWifiStageIns.getPluginId())
